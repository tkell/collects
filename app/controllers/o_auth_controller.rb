class OAuthController < ApplicationController
  ...
  def authorize
    ...
    if provider == :discogs
      ...
      body = {}
      request_token = consumer.get_request_token({"oauth_callback" => config[:user_agent]}, body, {"User-Agent" => config[:user_agent]})
      ...
      redirect_to discogs_auth_url # opens in new window
      return
    end
  end
  ...
  def callback
    ...
    linked_account.accces_token = access_token.token
    linked_account.accces_token_secret = access_token.secret
